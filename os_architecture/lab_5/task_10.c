...
int main() {
    char *path_to_server_key_file =
            "/Users/vladislavdankevich/CLionProjects/untitled/os_architecture/lab_5/server_queue_task_10";
    ...
    for (long i = 100; i < 110; i++) {
        ...
        if (pid == 0) {
            ...
            while (1) {
                ...
                if (msgrcv(server_msqid, &msg, sizeof(msg.mtext), type, MSG_NOERROR) == -1) {
                    perror("msgrcv");
                    exit(EXIT_FAILURE);
                }
    ...
    while (1) {
        ...
        if (msgrcv(server_msqid, &msg, sizeof(msg.mtext), 50, MSG_NOERROR) == -1) {
            perror("msgrcv");
            exit(EXIT_FAILURE);
        }
        ...
        for (long i = 100; i < 110; i++) {
            ...
            if (msgsnd(server_msqid, &new_msg, sizeof(new_msg.mtext), 0) == -1) {
                perror("msgsnd");
                exit(EXIT_FAILURE);
            }
        }
